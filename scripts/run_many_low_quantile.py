...
def parse_args():
    parser = argparse.ArgumentParser(description="Automate trading inference and parse Low Quantile logs.")
    ...
    parser.add_argument('--model', type=str, default='v2_btc_low', help='mode name')
    # 清理旧日志选项
...
def update_csv_file(args, data_map):
    ...
    try:
        ...
        # 初始化新列
        new_cols = ['Low-q0.05', 'Low-q0.2', 'Low-q0.4', 'Low-q0.6']
        for col in new_cols:
        ...
        for date_str, info in data_map.items():
            ...
            if mask.any():
                idx = df.index[mask]
                df.loc[idx, 'Low-q0.05'] = info['Low-q0.05']
                df.loc[idx, 'Low-q0.2'] = info['Low-q0.2']
                df.loc[idx, 'Low-q0.4'] = info['Low-q0.4']
                df.loc[idx, 'Low-q0.6'] = info['Low-q0.6']
                count += 1
