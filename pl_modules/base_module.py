...
class BaseModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.target_idx = None              # ADDED FOR REVIN: set later
        self.mse = nn.MSELoss()
        self.l1 = nn.L1Loss()
        self.mape = MAPE()
    ...
    def forward(self, x, y_old=None):
        ...
        # MODIFIED FOR REVIN: model now outputs (b, num_features, 1)
        model_out = self.model(x)                     # (b, f, 1)
        model_out = model_out.squeeze(-1)             # (b, f)
        y_hat_norm = model_out[:, self.target_idx]     # (b,)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/mse", mse.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=False)
        self.log("val/rmse", rmse.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=True)
        self.log("val/mape", mape.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=True)
        self.log("val/mae", l1.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=False)
        return {"val_loss": mse}
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test/mse", mse.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=False)
        self.log("test/rmse", rmse.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=True)
        self.log("test/mape", mape.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=True)
        self.log("test/mae", l1.detach(), sync_dist=True,
                 batch_size=self.batch_size, prog_bar=False)
